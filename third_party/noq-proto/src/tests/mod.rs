...
/// The endpoint decrements `active_connections` once per Draining with no
/// per-connection dedup; a second Draining underflows the counter (panic with
/// overflow checks, hang on `wait_all_draining` without them).
#[test]
fn regression_double_stateless_reset_single_draining() {
    let _guard = subscribe();
    let mut key_material = vec![0; 64];
    let mut rng = rand::rng();
    rng.fill_bytes(&mut key_material);
    ...
    let (client_ch, _) = pair.connect();
    pair.drive(); // Flush any post-handshake frames
    while pair.client_conn_mut(client_ch).poll_endpoint_events().is_some() {}

    // Fresh server endpoint with the same reset key has no connection state, so
    // it answers the client's packet with a stateless reset.
    pair.server.endpoint = Endpoint::new(endpoint_config, Some(Arc::new(server_config())), true);
    pair.client.connections.get_mut(&client_ch).unwrap().ping();
    pair.drive_client();
    pair.drive_server();

    // One reset should now be queued for the client. Duplicate it so the
    // connection sees two reset datagrams in a single processing batch — the
    // production trigger when a restarted peer emits several resets at once.
    assert_eq!(
        pair.client.inbound.len(),
        1,
        "expected a single stateless-reset datagram from the server"
    );
    let first = pair.client.inbound.front().expect("reset inbound");
    let second = util::Inbound {
        recv_time: first.recv_time,
        ecn: first.ecn,
        packet: first.packet.clone(),
        remote: first.remote,
        dst_ip: first.dst_ip,
    };
    pair.client.inbound.push_back(second);

    pair.client.drive_incoming(pair.time);
    let events = pair
        .client
        .conn_events
        .remove(&client_ch)
        .expect("client should have connection events for the resets");
    assert!(
        events.len() >= 2,
        "need at least two reset ConnectionEvents, got {}",
        events.len()
    );

    for event in events {
        pair.client_conn_mut(client_ch).handle_event(event);
    }

    let mut draining = 0u32;
    let mut drained = 0u32;
    while let Some(ev) = pair.client_conn_mut(client_ch).poll_endpoint_events() {
        if ev.is_draining() {
            draining += 1;
        }
        if ev.is_drained() {
    ...
    assert_eq!(
        draining, 1,
        "duplicate Draining would underflow endpoint active_connections"
    );
    assert_eq!(drained, 1);
