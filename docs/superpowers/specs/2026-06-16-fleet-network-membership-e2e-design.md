...
| D9 | Join bootstrap + token | Bump `InviteToken` to **v5**, adding `grant: MembershipRecord` + `network: NetworkRecord` (both admin-signed, types already in `network_token.rs`). Join verifies both admin_sigs, requires `grant.member_did == local DID`, `grant.status=="active"`, `grant.network_id == network.network_id == token.network_id` — admission from the **signed payload**, not the (non-signature-bound) `PeerRegistry` rows. **Issuer policy (cut-4 decision):** v1 = admin-issued only (`token.issuer_did == network.admin_did`, verified against `network.admin_sig`); member-issued invites (with an included issuer-membership proof) are a deferred extension. The token proves the *grantee*; this clause also pins the *issuer*. |
...
- **Layer 2 — data-plane desired collection (NEW):** a separate operator-owned `DataPlanePairingDesired` collection (name TBD) holds the conversation/delegation star rows (own unique `peer_id`, so no Layer-1 desired collision). Operator-owned; carries the `conversation` (PeerDid/Push) template.
- **Merged install per peer (spike-resolved, §6a):** defradb holds one replicator per peer. So the reconciler **unions both layers' desired rows per peer address** into a single `add_replicator` (control collections unfiltered + conversation collections peer-DID-filtered, via the per-collection filter map), always installing the full union (add_replicator replaces). One `PeerPairingApplied` row per peer records the union — no schema change.
- **Membership master gate (D11):** the merged install is gated by the shared `decideMaterializable(peer)`; a non-member (or revoked, via `revoke_*` tombstone) peer is dropped entirely — both layers retract on the next sweep, no separate prune.
- **Compatibility:** existing discovery writes `source="registry"` (`discovery.rs:47`); cut 5 adds `source="network"` beside it, and the Layer-2 rows live in the new collection. Plan states precedence/migration so `registry` (transitional) and `network` (membership-fenced target) desired rows never fight over a `peer_id`, and the union step orders Layer-1 + Layer-2 deterministically.
...
3. **Serial join** for each Pi (i=2..5): P1 `grant <did_i>`, P1 `pairings invite --template network-control` (the enrollment invite uses the **`network-control`/`discovery` on-ramp template, NOT `conversation`** — `join` writes a `PeerPairingDesired`, so it must seed the Layer-1 substrate edge, not a data-plane edge), carrying `network_id` + the signed grant (D9). Pi `pairings join <token>` through the real gate (sig → freshness → network-match → **membership admission from the signed payload** → nonce burn → `PeerPairingDesired` source=operator/network on the control-plane template). Reciprocal where required.
...
- **Merged-install correctness** (per-address question resolved in §6a → merged single replicator). Residual risk is in the union step: it must always install the *full* union (add_replicator replaces), so a bug that installs only one layer silently drops the other's collections. Conformance must assert the union, and the diff must teardown-collections via `remove_replicator_collections` (collection-scoped), not whole-peer teardown, when only one layer's collections change.
- **New `DataPlanePairingDesired` collection.** Adds a schema + a desired-row source that the reconciler must merge into the existing per-peer diff/install engine — not fork it. Shares the `decideMaterializable` gate (D11).
- **Source-partition coexistence.** Cut 5 adds `source="network"` beside legacy `source="registry"` in `discovery.rs`. The plan defines precedence/migration so registry-derived rows degrade cleanly as membership-fenced rows take over.
...
## 6a. Replicator/applied-state spike — RESOLVED (merged single replicator)

Spiked against pinned `defradb.rs` (rev `bddfcea5`). Findings:
- **One `ReplicatorInfo` per peer ID** — `ReplicatorRegistry` is `BTreeMap<String, ReplicatorInfo>` keyed by peer id (`crates/p2p/src/bitswap/registry.rs:19`). Two replicators to one peer are impossible.
- **`add_replicator` REPLACES** the whole record (`registry.rs:54-64`), so a separate Layer-2 install would wipe Layer-1's collections.
- **Filters are per-collection within a single replicator** — `ReplicationFilters = BTreeMap<collection_id, ReplicationFilter>` (`crates/p2p/src/replicator.rs:51-108`); `get_replicators` exposes them; `remove_replicator_collections` is collection-scoped (`registry.rs:86-104`).

**Chosen install model: merged single replicator per peer.** The reconciler unions, per peer address, Layer-1 control collections (unfiltered) + Layer-2 conversation collections (peer-DID-filtered) into **one** `add_replicator` call with a per-collection filter map. Because `add_replicator` replaces, the reconciler always installs the **full union** (never an incremental add that would clobber the other layer).

**Consequences baked into the plan:**
- **No `PeerPairingApplied` schema change** — its `peer_id` key already matches defradb's one-replicator-per-peer model; `replicator_filter` (already a per-collection `PairingFilters` map) records the installed union.
- **No `diff.rs` actual-read change** — actual stays address+collections (filter unreadable from remote, as documented); the `(address, filter)` identity remains fenced reconciler-side via the applied row.
- **The two desired collections are merged at reconcile time, not at the schema.** Clean ownership stays (network-derived vs operator desired rows); the *install* unions them per peer behind the shared `decideMaterializable` gate (D11).
- **Lean alignment (cut-5):** `PairingReconcile.ReplicatorId = (address, Option filter)` (`proofs/Proofs/PairingReconcile/State.lean:29-45`) models a single scope filter per replicator; the merged model needs a per-collection filter map. Cut-5 planning checks whether the model generalizes cleanly (it already carries `PairingFilters` per-collection) or needs a small lemma update — Lean-first.
