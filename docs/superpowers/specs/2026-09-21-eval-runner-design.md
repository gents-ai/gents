...
| Trial isolation in 2a | Embedded-first. At freeze time the embedded executor refuses any subject whose tools configuration grants any host bash mode other than `Off` *(widened at execution, M2 ruling R25: `ReadOnly` is an unconfined argv allowlist with inherited network, and the workspace modes are sandboxed only on macOS through `sandbox-exec`, which the executor cannot verify on the host; the workspace modes are re-admitted only by an executor that can prove enforcement, a 2b item)*. Datastore and file tools under a workspace root inside the trial home are allowed | `BashMode::Unrestricted` runs commands unmodified with the runner's privileges and environment, including API keys. Only the workspace modes get `sandbox-exec`, and only on macOS. A candidate prompt is untrusted text that steers an agent, so "embedded is not a sandbox" has to be an enforced boundary, not a caveat |
