...
- **Bridge** (`AgentToolCall`): one tool call ↔ one child via `child_request_id`; lifecycle `Pending → Running → Completed/Failed/Cancelled` (`tool_call_lifecycle/transition/bridge.rs`: `bridge_complete` / `bridge_failure` / `bridge_cancel_cascade`).
...
- **`Workflow/FanOut.lean`** (cut 1) — model a fan-out group as a set of `Background.BridgedState` bridges sharing one orchestrator parent, plus an optional synthesis bridge. Transition `synthesis_spawn` is **enabled only when every group child has reached a `ChildTerminal` state**. Theorem **`barrier_completeness`**: in any reachable state where the synthesis request exists, all N group children are terminal (no partial-fan-in synthesis). Composes on `bridge_complete`/`bridge_failure`.
...
| D4 (Q3) | Barrier persistence | **Pure projection, no new collection** for cut 1. Barrier-completeness projects from existing rows: the N children share `caused_by_parent_request_id` (the orchestrator request) and the synthesis request exists only after all N are terminal. The `Collection` enum + `ApplyReconcile`/`Collections.lean` parity contract stay **untouched**. A minimal round/group discriminator *field* (not a new collection) is added only if `loop_until_done` (cut 4) needs round discrimination. |
| D5 | Synthesis shape | **Distinct synthesis subagent whose result returns to the orchestrator continuation** (user steer; Claude Code's `const r = await parallel(...); return synthesize(r)` shape). The synthesis is a *spawned child request* (depth+1, fed the N child answers) — this is what makes barrier-completeness assertable from rows (`synthesis exists ⟹ ∀ child terminal ∧ synthesis.created_at ≥ max(child.terminal_at)`). Its result then bridges back as the `fan_out_and_synthesize` tool result, so the orchestrator continues its own loop. |
| D6 (Q2) | Budget / fan-out caps | Fan-out **width reuses `maxBackgroundedPerParent = 8`** (no new width cap for cut 1; N ≤ 8). **Depth reuses `maxSubagentDepth = 3`**: orchestrator at depth d → children + synthesis at d+1, so an orchestrator must sit at depth ≤ 2 to fan out — a real constraint we state and conformance-test. `loop_until_done` rounds are *sequential* fan-outs (depth stays flat per round) bounded by a new **workflow-level total-spawn budget** = the termination proof's `budget` (added in cut 4). |
...
| D9 | This session's scope | **Three artifacts only:** this design doc, the cut plan, and the failing/ignored aspirational test skeleton. No Lean/Rust this session. |
...
**Assertions — purely from durable rows (the projection):**
- *Fan-out lineage:* exactly 3 child `AgentRequest`s share `caused_by_parent_request_id = orchestrator_request` with `caused_by_trigger_kind = "subagent"`; each has a distinct `caused_by_parent_tool_call_id` bridge in `AgentToolCall`.
- **barrier-completeness (the headline):** the synthesis `AgentRequest` exists **and** every one of the 3 fan-out children is in a terminal `lifecycle_state` **and** `synthesis.created_at ≥ max(child.terminal_at)`. No state is observable where synthesis exists with a non-terminal sibling.
- *Synthesis return:* the synthesizer's terminal result bridges back (`bridge_complete`) as the `fan_out_and_synthesize` tool result; the orchestrator parent terminates cleanly with no orphaned bridges.
- *Budget/depth:* orchestrator at depth 0, children + synthesis at depth 1; N=3 ≤ 8.

**Cross-deployment fleet shape (cut 5):** the #513 coordinator becomes the orchestrator; the 4 subagent deployments are the fan-out targets; barrier-completeness must hold over **replicated** child-terminal rows (the coordinator learns child terminal via replication, as the existing bridge-complete path already does). Same assertions, projected fleet-wide.

The **first wall** is intentional: the skeleton references a `fan_out_and_synthesize` tool that does not exist, so it fails to compile/run — naming cut 1's first affordance.
...
### Cut 0 — Failing aspirational test + docs (this session, Lean-neutral)
Design doc (this file) + plan doc + the ignored/env-gated `workflow_orchestration_live.rs` skeleton encoding §6. No transition/invariant touched → no Lean. The wall: `fan_out_and_synthesize` undefined.

### Cut 1 — `fan_out_and_synthesize` end-to-end (the foundation)
- **Lean:** `Proofs/Workflow/FanOut.lean` — `barrier_completeness` (0 sorry), composing on `Background` bridges. Register conformance witnesses (legal: synthesis-after-all-terminal; illegal: synthesis-before-any-terminal).
- **Conformance:** Rust tests drive the witness cases against the projected barrier predicate.
- **Rust:** a common `OrchestrationPrimitive` trait + the `fan_out_and_synthesize` tool (thin surface, D3); a runtime sub-engine that spawns N bridges (reusing `SubagentSource`), projects the barrier from lineage (D4), spawns the synthesis child (D5), bridges its result back. Other three primitives stubbed behind the trait.
...
- **Cross-deployment barrier (cut 5)** depends on child-terminal rows replicating back to the orchestrator — the same path #513's `bridge_complete` already exercises, but the *barrier* now waits on N of them. Replication lag becomes a convergence-polling concern (bounded deadlines, no sleeps).
- **`loop_until_done` round discrimination** (cut 4) may force a minimal grouping/round field on `AgentRequest`/`AgentToolCall`. That is a schema touch (not a new collection) — keep it off the `Collection`-enum parity path; fence by conformance.
