...
- **`Workflow/FanOut.lean`** (cut 1) — model a fan-out group as a set of `Background.BridgedState` bridges sharing one orchestrator parent **and one `workflow_group_id`** (= the `fan_out_and_synthesize` tool call id, D4), plus an optional synthesis bridge tagged `workflow_role = synthesis`. Transition `synthesis_spawn` is **enabled only when every group child has reached a `ChildTerminal` state** — terminal includes `failed`/`dead`/`interrupted`, *not just* `completed` (D10). Theorem **`barrier_completeness`**: in any reachable state where the synthesis bridge exists, all N group children are terminal (no partial-fan-in synthesis). The synthesis input is the N **structured outcomes** (success or failure record), so the theorem is over *terminality*, not *success*. Composes on `bridge_complete`/`bridge_failure`.
...
| D10 | Failed-child semantics | **Structured failure record; synthesis always runs post-barrier** (review fix). A child that reaches a non-`completed` terminal (`failed`/`dead`/`interrupted`) becomes a structured failure outcome in the synthesis input; the barrier counts it as terminal and synthesis still runs over the N outcomes. Mirrors Claude Code's `parallel()` + `.filter(Boolean)` resilience, and keeps `barrier_completeness` a statement about *terminality*, not *success*. (Alternative — fail-the-primitive-without-synthesis — is rejected: it discards partial work and would make the theorem about success, coupling the barrier to model output quality.) |
| D11 | Tool surface exposure | **New `orchestration_enabled: Boolean` field on `ToolSelection`, default off** (review fix — there is no orchestration selector today). Orchestration is a *distinct privilege* (the power to drive a fleet), so it is **not** auto-derived from subagent enablement and **not** folded into `write_tools` (file-write tools). It **composes with** `subagent_spawn_enabled` (and, for fleet fan-out, `subagent_allow_cross_deployment`): orchestration tools are added to the per-behavior surface at reconcile time only when `orchestration_enabled ∧ subagent_spawn_enabled`, since the primitives materialize through the subagent path. Adding a field to an existing apply-path collection touches the config renderer, not the `Collection`/parity contract. |
...
- **barrier-completeness (the headline):** the synthesis bridge (`workflow_role = synthesis, workflow_group_id = G`) exists **and** all 3 fan-out bridges are in a terminal `lifecycle_state` (`Completed`/`Failed`/`Cancelled`) **and** `synthesis_bridge.started_at ≥ max(fan_out_bridge.completed_at)`. No state is observable where the synthesis bridge exists with a non-terminal sibling.
- *Failed-child (D10):* if a fan-out child is `Failed`/`Cancelled`, the barrier still admits synthesis and the synthesis input carries its structured failure record — synthesis runs over 3 outcomes regardless of per-child success.
...
**The first wall is a *run* failure, not a compile failure** (review fix). Rust `#[ignore]` tests still compile with the package, and the suite is gated on the full build (§9), so the skeleton must compile clean: it uses only existing harness helpers, string tool names (`"fan_out_and_synthesize"`), and GraphQL row queries — no reference to not-yet-defined Rust symbols. It passes the suite as `#[ignore]` and **fails only when explicitly run** with `DEFRA_AGENT_LIVE_WORKFLOW=1 -- --ignored`: the tool does not exist, so the orchestrator never emits a `workflow_group_id`-tagged group and the barrier assertions fail. That run failure names cut 1's first affordance.
...
- **Rust:** a common `OrchestrationPrimitive` trait + the `fan_out_and_synthesize` tool (thin surface, D3), gated by `orchestration_enabled ∧ subagent_spawn_enabled`; a runtime sub-engine that spawns N group-tagged bridges (reusing `SubagentSource`), projects the barrier from lineage (D4), spawns the synthesis child with the N structured outcomes (D5/D10), and completes the orchestration tool call with its result. Other three primitives stubbed behind the trait.
