...
- **`BridgeTransition.recoverPaired` requires the same step that flips the response to error to also flip the request to failed.** This is reading (a) — *intentional*: the bridge represents the *post-recovery atomic state* from the model's perspective, not the runtime's mutation atomicity. In Rust, `RequestLifecycle::recover_all` runs `recover_stuck_responses` and `recover_stuck_requests` as the same startup-sweep pass; the intermediate state (response = error, request = processing) is transient and observed only by the next-iteration query inside that sweep call. The model collapses this to one bridge step because the only thing downstream consumers (S6, the recovery liveness story) need is the *post-sweep* paired state. If the runtime ever splits the two sweep halves across separate recovery passes (so the intermediate state becomes durable across crashes), reading (a) becomes wrong and the bridge must split into two transitions — a documented obligation, not silent.

- **`finalizeError`'s four legal reasons carry no preconditions beyond the `streamIdleTimeout` deadline check.** `.inferenceFailed`, `.finalizeRequestedError`, and `.interrupted` are fired unconditionally from external runtime signals (the daemon's inference error path, `finalize_existing_request_error`, and the interrupt flow respectively); the model has nothing to constrain them on. Only `streamIdleTimeout` carries a typed precondition (`now > streamIdleDeadline`) because it's the one timeout the deadline audit specifically calls out. This is intentional — flagged in §Risks below.
...
- **`finalizeError` accepts three reasons with no preconditions**: `.inferenceFailed`, `.finalizeRequestedError`, and `.interrupted` carry no model-level constraints because they are fired by runtime-external signals (LLM error path, `finalize_existing_request_error` call, interrupt flow). This is intentional and matches the operational shape: the model is asserting that *if* the runtime emits one of these reasons, the resulting transition is legal — it does not constrain *when* the runtime is allowed to emit them. If a future spec refinement wants to add operational preconditions (e.g., `interrupted` requires `pre.interruptedAt = some _`), this is the location to thread them.

- **`BridgeTransition.recoverPaired` atomicity reading**: the bridge collapses two non-atomic Rust mutations (`recover_stuck_responses` + `recover_stuck_requests`, both inside `RequestLifecycle::recover_all`) into one model step. The transient state (response = error, request = processing) is observable in principle between the two SQL calls but is consumed by the same recovery pass before any downstream observer (subscription, lifecycle counter) sees a durable inconsistency. If the runtime ever splits these sweeps across separate recovery passes — making the transient state durable — this collapse becomes incorrect and the model must split `recoverPaired` into two ordered transitions. The current modeling matches Rust's `recovery.rs` shape today.
