...
def write_fixture(out_dir: Path, capture: dict[str, Any]) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "langgraph_state_history.capture.json"
    payload = {
        "source": {
            "system": "langgraph",
            "package": "langgraph",
            ...
            "api": ["StateGraph", "InMemorySaver", "get_state_history"],
        },
        "native": {
            "thread_id": THREAD_ID,
            "graph": {
        ...
        "envelope": build_projection(capture),
    }
...
def main() -> None:
    ...
    capture = run_langgraph_capture()
    path = write_fixture(args.out_dir, capture)
    print(path)
