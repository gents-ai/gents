...
**Cut-1 deviation (recorded):** the common `OrchestrationPrimitive` trait + `pipeline`/`verify`/`loop_until_done` stubs (Task 1.4 Step 2 / design D8) are intentionally deferred to cut 2. A trait with a single implementation (and dead `#[allow(dead_code)]` stub types) adds abstraction with no consumer; the real shared seam in cut 1 is `build_orchestration_tools`/`orchestration_tool_names` + the `OrchestrationToolConfig` gate. The trait lands when cut 2 introduces a second implementation to factor against. (Noted in `toolset/orchestration.rs` module docs.)
