...
**Field-source note:** SDL field names MUST match the cut-1 Lean entities (spec §4): `Network{networkId,adminDid,adminSigValid→admin_sig}`, `Membership{networkId,memberDid,active→status,signedBy,...}`, `Endpoint{did,peer→node_id,...}`, `JoinRequest{networkId,candidateDid,...}`. Cut 1 modeled `active:Bool`; the SDL uses `status: String` (active|revoked) per spec §4 — the reconciler (cut 5) maps `status=="active"` ↔ the model's `active`.
...
**Out of scope (later cuts):** writing/reading these rows, signing them with a real DID key, the pointer mint/verify in the CLI (cut 3), join/approve document flow (cut 4), reconciler reading `deriveMaterializable` over these docs (cut 5). **Placeholder scan:** all SDL, consts, structs, migration fns, template entry, and gate commands are concrete; count-test literals are computed-from-actual (not guessed) per the cut-1 lesson. **Type consistency:** SDL field names ↔ spec §4 ↔ cut-1 Lean entities; signing-struct fields ↔ SDL signed fields (record content minus the `*_sig`, which becomes the struct's zeroed `sig`); `NetworkPointer` mirrors `InviteToken`.
