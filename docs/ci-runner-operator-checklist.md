...
unset by Cargo. CI sets both wrapper environment forms and temporarily prepends
the absolute pass-through path to the repository's tracked
`.cargo/config.toml`. The nearest config is also honored by recursive Cargo
processes after build-script environment filtering; the next checkout's
`git reset --hard` restores the tracked file even after a canceled job. Cache
setup fails if the repository gains its own `[build]` settings so the injection
cannot silently conflict. The installed sccache rejects incremental rustc
invocations instead of passing them through. A Studio A/B on `gents
--all-targets` measured a representative source edit at 14.6 seconds with warm
incremental state versus 5m54s with sccache and incremental disabled.
