...
unset by Cargo. The installed sccache rejects incremental rustc invocations
instead of passing them through. A Studio A/B on `gents --all-targets` measured
a representative source edit at 14.6 seconds with warm incremental state versus
5m54s with sccache and incremental disabled. Establishing the incremental graph
took 9m26s, so the already-short support suite retains sccache instead.
