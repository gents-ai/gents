...
```sh
sudo apt install ./gents-desktop_0.17.1_amd64.deb
```
...
```sh
chmod +x gents-desktop_0.17.1_x86_64.AppImage
./gents-desktop_0.17.1_x86_64.AppImage
```

AppImage support still depends on host graphics/display facilities. If FUSE is
unavailable, use `APPIMAGE_EXTRACT_AND_RUN=1 ./gents-desktop_0.17.1_x86_64.AppImage`.
