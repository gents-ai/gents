...
pub(crate) async fn load_agent_principal_record(
    ...
    Ok(serde_helpers::first_row_with_doc_id(resp.data.as_ref(), "AgentPrincipal"))
}
...
pub(crate) async fn load_agent_behavior_record(
    ...
    Ok(serde_helpers::first_row_with_doc_id(resp.data.as_ref(), "AgentBehavior"))
}
...
pub(crate) async fn list_agent_behavior_records(
    ...
    Ok(serde_helpers::rows_with_doc_id(resp.data.as_ref(), "AgentBehavior"))
}
...
pub async fn upsert_agent_principal(
    ...
    let fallback_display_name = serde_helpers::default_display_name_for_did(agent_did);
    let display_name =
        serde_helpers::normalize_optional_string(display_name).unwrap_or(fallback_display_name.as_str());
    let escaped_display_name = escape_graphql_string(display_name);
    let escaped_default_behavior_id =
        escape_graphql_string(serde_helpers::normalize_optional_string(default_behavior_id).unwrap_or_default());
