...
#[tokio::test]
async fn run_agent_uses_backend_specific_api_key_env_var_for_startup_probe() {
    let node = test_node().await;
    ensure_runtime_schemas(node.as_ref()).await.unwrap();
    let identity = Arc::new(test_identity("startup-probe-backend-auth"));
    let mock_endpoint =
        MockModelEndpoint::start_with_required_bearer("default", Some("backend-key")).unwrap();
    bind_default_behavior_backend(
        node.as_ref(),
        identity.did(),
