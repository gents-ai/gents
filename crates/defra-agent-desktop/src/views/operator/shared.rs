...
pub(super) fn normalize_optional_owned(value: &str) -> Option<String> {
...
pub(super) fn parse_optional_i64(field: &str, value: &str) -> Result<Option<i64>> {
...
pub(super) fn parse_required_positive_i64(field: &str, value: &str) -> Result<i64> {
...
pub(super) fn parse_optional_f64(field: &str, value: &str) -> Result<Option<f64>> {
    match normalize_optional_owned(value) {
        Some(value) => value
            .parse::<f64>()
            .map(Some)
...
pub(super) fn parse_optional_rfc3339(field: &str, value: &str) -> Result<Option<String>> {
    match normalize_optional_owned(value) {
...
pub(super) fn bool_word(value: Option<bool>) -> &'static str {
    if value == Some(true) {
        "on"
    } else {
        "off"
...
pub(super) fn scheduled_task_is_due(row: &ScheduledTaskRow) -> bool {
...
pub(super) fn scheduled_task_next_run_label(row: &ScheduledTaskRow) -> String {
    match row.next_run_at.as_deref().and_then(parse_task_timestamp) {
...
pub(super) fn parse_task_timestamp(value: &str) -> Option<DateTime<Utc>> {
    DateTime::parse_from_rfc3339(value)
        .ok()
        .map(|parsed| parsed.with_timezone(&Utc))
}

pub(super) fn summarize_request_content(content: &str, fallback_id: &str) -> String {
...
pub(super) fn truncate_line(value: &str, max_chars: usize) -> String {
...
pub(super) fn abbreviate_identifier(value: &str) -> String {
    if value.len() <= 10 {
        value.to_string()
    } else {
