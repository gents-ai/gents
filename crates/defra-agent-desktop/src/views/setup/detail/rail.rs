...
pub(super) fn show_rail(
    ...
    let Some(client) = client else {
        views::card(
            ...
            "The desktop client must be online before deployment metadata can render.",
        );
        return;
    };
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        let heading_badge = selected_peer
            .as_ref()
            .map(|peer| if peer.connected { "connected" } else { "saved" })
            .unwrap_or("idle");
        views::sidebar_heading(ui, "Deployment Detail", Some(heading_badge));
        ui.add_space(10.0);
        ...
        let Some(peer) = selected_peer else {
            views::card(
                ...
                "Select a deployment from the sidebar to inspect connection metadata and local persistence details.",
            );
            return;
        };
        ...
        ui.group(|ui| {
            ...
            labeled_value(ui, "Deployment Label", &peer.label);
            labeled_value(ui, "Deployment ID", &peer.record_id);
        ...
        ui.group(|ui| {
            ...
            labeled_value(ui, "Local Node ID", client.local_peer_id());
            labeled_value(
                ui,
                "Listen Address",
                client
        ...
        ui.group(|ui| {
            ...
            labeled_value(ui, "Remote Schema Match", "not exposed by the node API");
        });

        ui.add_space(12.0);
        if audit::button(ui, audit::targets::SETUP_REMOVE, "Remove Deployment").clicked() {
