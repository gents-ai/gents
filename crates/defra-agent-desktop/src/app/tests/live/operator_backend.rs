...
fn wait_for_live_backend_section_ready(
    ...
    if driver.app.state.operator.selected_agent_did.as_deref()
        != Some(deployment.agent_did.as_str())
    {
        driver.click_target(&audit::targets::operator_agent(&deployment.agent_did));
    }
    if driver.app.state.operator.selected_section != OperatorSection::Backends {
        driver.click_target(&audit::targets::operator_section(OperatorSection::Backends));
    }

    wait_for_value(
        "live operator backend section ready for new document",
    ...
    driver.wait_for_target(
        "live operator new backend button",
        Duration::from_secs(20),
...
fn desktop_app_live_operator_creates_backend_and_uses_it_for_inference() -> Result<()> {
...

    {
        ...
        driver.click_compact_target(audit::targets::OPERATOR_NEW)?;
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                anyhow::bail!(
                    "timed out waiting for live operator new backend draft active; section={:?} selected_entity_id={:?} draft_origin={:?} draft={:?} pending_shell_actions={:?}",
                    driver.app.state.operator.selected_section,
                    driver.app.state.operator.selected_entity_id,
                    driver.app.state.operator.draft_origin,
...

    {
        ...
        assert_backend_draft_fields(driver, &backend);
        if driver.app.state.operator.selected_section != OperatorSection::Behaviors {
