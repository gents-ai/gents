...
fn desktop_app_live_operator_creates_profile_and_rebinds_behavior() -> Result<()> {
    ...
    let profile = OperatorInferenceProfileForm {
...

    {
        ...
        wait_for_value(
            ...
            || {
                ...
                (driver.app.state.operator.selected_section == OperatorSection::InferenceProfiles
                    && driver.app.state.operator.selected_entity_id.as_deref()
                        == Some(profile.profile_id)
                    && matches!(
                        driver.app.state.operator.draft.as_ref(),
                        Some(OperatorDraft::InferenceProfile(_))
                    ))
                .then_some(texts)
            },
        )?;
