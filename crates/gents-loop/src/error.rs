...
/// "Invalid `signature` in `thinking` block"; OpenAI-style Responses providers
/// report undecryptable `encrypted_content`. Resending the same body fails the
/// same way, so this is never resampled.
