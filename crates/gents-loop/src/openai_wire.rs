...
impl OpenAiWireApi {
    ...
    /// Logs when a stored wire choice cannot affect this provider. Public so
    /// the native registry can call it; the guest loop does not log on its own.
    pub fn warn_if_ignored(
        provider_kind: BackendProviderKind,
        configured: Option<Self>,
        backend_id: &str,
    ) {
