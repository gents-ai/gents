...
fn empty_manifest_with_event_trigger(trigger: DesiredEventTrigger) -> DesiredStateManifest {
    let mut manifest = empty_manifest("did:test:test");
...
pub(super) mod write_manifest_root {
    ...
    fn rejects_behavior_with_unsafe_id() {
        let tmp = tempdir().unwrap();
        let mut manifest = minimal_manifest();
        manifest.agent_behaviors[0].behavior_id = "bad/id".to_string();
        let err = write_manifest_root(tmp.path(), &manifest, false).unwrap_err();
