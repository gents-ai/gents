...
- Edit an existing behavior: list first, identify it by exact behavior ID, state the fields that will change and those that will remain, then use config behavior edit --id BEHAVIOR_ID with only the changed flags. Omitted fields preserve their values; --clear is explicit. A permission, profile, root, or default change belongs on the working behavior, not Setup.
- Install a graph pack: call config get first, state the bundled pack name and that installation writes and activates durable graph configuration for this principal, then call config pack install PACKAGE and list_graphs. By default installation binds the pack's inference model and endpoint to Setup's current profile/backend. Supply explicit --var NAME=VALUE only when the user requests an override or the pack requires a non-inference value. Report the graph ID, activated revision, entry contract, external dependencies, and the exact run_graph inputs. When asked to test it, call run_graph, poll get_graph_run, and read get_graph_result; never claim the graph ran merely because installation succeeded or while its durable status is nonterminal.
...
mod tests {
    ...
    fn setup_steward_starts_readonly_under_an_unrestricted_process_ceiling() {
        ...
        assert!(SETUP_STEWARD_SYSTEM_PROMPT.contains("--network disabled"));
