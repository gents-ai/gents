...
use crate::{
    fetch_request_view, hydrate_materialized_response_content, materialized_response_diagnostic,
    optional_response_row, post_graphql, print_json, resolve_graphql_endpoint, resolve_request_id,
    response_query, wait_for_terminal_response, MaterializedResponsePresentation,
};
...
pub(crate) async fn response_show(args: ResponseShowArgs) -> Result<()> {
    ...
    let query = response_query(&request)?;
    let mut response = post_graphql(&graphql, &query).await?;
    optional_response_row(&response)?;
