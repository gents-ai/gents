...
| `RenderedRequest` | `capture_key`, `request_doc_id`, `request_commit_cid`, `request_id`, `session_id`, `capture_scope`, `turn_index`, `attempt`, `request_json`, `prompt_hash`, `tools_hash`, `provenance_json` | one durable fact per provider attempt: the exact HTTP request body and exact claimed request snapshot, persisted before send | `rendered_request::transport::RenderedRequestCapturingHttpClient`, the innermost transport in every provider stack, through `rendered_request::sink::DefraRenderedRequestSink` (installed by default) | trace projections, capture reconstruction (still `CapturedOnly` until all source versions and authorization evidence are pinned) |
