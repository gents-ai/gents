...
pub use gents_schemas::{
    ...
    AGENT_NETWORK, AGENT_NETWORK_NAME, AGENT_OUTPUT_SEGMENT, AGENT_OUTPUT_SEGMENT_NAME,
    AGENT_PRINCIPAL, AGENT_PRINCIPAL_NAME, AGENT_REQUEST, AGENT_REQUEST_NAME, AGENT_RUNTIME,
    AGENT_RUNTIME_NAME, AGENT_SESSION, AGENT_SESSION_NAME, AGENT_TOOL_CALL, AGENT_TOOL_CALL_NAME,
    ALL as AGENT_ALL, ALL_COLLECTION_NAMES as AGENT_COLLECTION_NAMES,
    BRANCHABLE_COLLECTION_NAMES as BRANCHABLE_AGENT_COLLECTION_NAMES, CALLBACK, CALLBACK_BINDING,
    CALLBACK_BINDING_NAME, CALLBACK_INVOCATION, CALLBACK_INVOCATION_NAME, CALLBACK_MODULE,
    CALLBACK_MODULE_NAME, CALLBACK_NAME, CALLBACK_RESULT, CALLBACK_RESULT_NAME, CHAIN_KEY_BINDING,
    CHAIN_KEY_BINDING_NAME, COMPACTION_ENTRY, COMPACTION_ENTRY_NAME, DATASTORE_TOOL_SURFACE,
    DATASTORE_TOOL_SURFACE_NAME, DATA_PLANE_PAIRING_DESIRED, DATA_PLANE_PAIRING_DESIRED_NAME,
    ENROLLMENT_OPERATOR_NONCE, ENROLLMENT_OPERATOR_NONCE_NAME, ETH_SUBMISSION, ETH_SUBMISSION_NAME,
    ETH_TOOL, ETH_TOOL_NAME, EVAL_DEFINITION, EVAL_DEFINITION_NAME, EVAL_RUN, EVAL_RUN_NAME,
    EVAL_TRIAL, EVAL_TRIAL_NAME, EVAL_VERDICT, EVAL_VERDICT_NAME, EVENT_GROUP_STATE,
    EVENT_GROUP_STATE_NAME, EVENT_SOURCE, EVENT_SOURCE_NAME, GOAL, GOAL_CREATION_CLAIM,
    GOAL_CREATION_CLAIM_NAME, GOAL_NAME, GRAPH_DEFINITION, GRAPH_DEFINITION_NAME, GRAPH_REVISION,
    GRAPH_REVISION_NAME, GRAPH_RUN, GRAPH_RUN_NAME, ISOLATED_WORKSPACE, ISOLATED_WORKSPACE_NAME,
    LOCAL_AUDIT_COLLECTION_NAMES, LOCAL_ONLY_COLLECTION_NAMES, MAILBOX_ITEM, MAILBOX_ITEM_NAME,
    NETWORK_ADMIN_PIN, NETWORK_ADMIN_PIN_NAME, NETWORK_AUTHORIZATION_REVISION,
    NETWORK_AUTHORIZATION_REVISION_NAME, NETWORK_ENROLLMENT_DECISION,
    NETWORK_ENROLLMENT_DECISION_NAME, NETWORK_ENROLLMENT_REQUEST, NETWORK_ENROLLMENT_REQUEST_NAME,
    NETWORK_ENROLLMENT_ROUTE_RECEIPT, NETWORK_ENROLLMENT_ROUTE_RECEIPT_NAME, OPTIMIZATION_JOB,
    OPTIMIZATION_JOB_NAME, PACK_INSTALLATION, PACK_INSTALLATION_NAME, PEER_ENDPOINT,
    PEER_ENDPOINT_NAME, PEER_PAIRING_APPLIED, PEER_PAIRING_APPLIED_NAME, PEER_PAIRING_DESIRED,
    ...
    PROVIDER_CONTEXT_REDUCTION, PROVIDER_CONTEXT_REDUCTION_NAME, RENDERED_REQUEST,
    RENDERED_REQUEST_NAME, REPOSITORY_PLACEMENT, REPOSITORY_PLACEMENT_NAME, SCHEDULE,
    SCHEDULE_NAME, SESSION_HYDRATION_REQUEST, SESSION_HYDRATION_REQUEST_NAME, SKILL, SKILL_NAME,
    SUBAGENT_TARGET, SUBAGENT_TARGET_NAME, TASK, TASK_NAME, TOOLS, TOOLS_NAME, TRIGGER,
    TRIGGER_NAME, WORKSPACE_BINDING, WORKSPACE_BINDING_NAME, WORKSPACE_PLACEMENT,
...
/// Every schema required by a full agent deployment, in registration order.
pub const ALL: &[&str] = &[
    AGENT_PRINCIPAL,
    AGENT_BEHAVIOR,
    COMPACTION_CONFIG,
...
mod tests {
    ...
    #[test]
    fn behavior_readiness_is_registered_but_not_broad_synced() {
        let index = ALL_COLLECTION_NAMES
            .iter()
            .position(|name| *name == AGENT_BEHAVIOR_READINESS_NAME)
            .expect("behavior readiness must be reserved in the protocol catalog");
        assert_eq!(ALL[index], AGENT_BEHAVIOR_READINESS);
    ...
    #[test]
    fn enrollment_route_receipt_is_registered_but_only_exact_pushed() {
        let index = ALL_COLLECTION_NAMES
            .iter()
            .position(|name| *name == NETWORK_ENROLLMENT_ROUTE_RECEIPT_NAME)
            .expect("route receipt must be reserved in the protocol catalog");
        assert_eq!(ALL[index], NETWORK_ENROLLMENT_ROUTE_RECEIPT);
        assert!(AGENT_COLLECTION_NAMES.contains(&NETWORK_ENROLLMENT_ROUTE_RECEIPT_NAME));
        assert!(!BRANCHABLE_COLLECTION_NAMES.contains(&NETWORK_ENROLLMENT_ROUTE_RECEIPT_NAME));
    }
}
