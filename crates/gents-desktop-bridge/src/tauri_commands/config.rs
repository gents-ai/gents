...
use crate::commands::{
    create_behavior_scaffold, delete_backend_config, delete_behavior_config, delete_context_config,
    ...
    save_skill_config, save_tool_service_config, save_tools_config, test_tool_service_config,
};
...
use crate::types::{
    ...
    BehaviorSaveRequest, BehaviorScaffoldCreateRequest, BehaviorScaffoldCreateResult,
    ContextDeleteRequest, DesktopClientSnapshot, InferenceProfileDeleteRequest,
    InferenceProfileSaveRequest, ScheduleDeleteRequest, SkillDeleteRequest, SkillSaveRequest,
    TaskDeleteRequest, ToolServiceDeleteRequest, ToolServiceSaveRequest, ToolServiceTestRequest,
    ToolServiceTestResult, ToolsDeleteRequest, ToolsSaveRequest, TriggerDeleteRequest,
};
...
#[tauri::command]
pub async fn desktop_behavior_scaffold_create<R: Runtime>(
    app: AppHandle<R>,
    request: BehaviorScaffoldCreateRequest,
    state: State<'_, DesktopAppState>,
) -> Result<BehaviorScaffoldCreateResult, BridgeError> {
    let Some(core) = current_core(&state) else {
        return Err(BridgeError::untyped("desktop client is not running"));
    };
    let behavior_id = create_behavior_scaffold(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    let snapshot = emit_config_update_and_snapshot(&app, &core, &state).await?;
    Ok(BehaviorScaffoldCreateResult {
        behavior_id,
        snapshot,
    })
}
...
#[tauri::command]
pub async fn desktop_tools_save<R: Runtime>(
    app: AppHandle<R>,
    ...
    save_tools_config(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    emit_config_update_and_snapshot(&app, &core, &state).await
}
...
#[tauri::command]
pub async fn desktop_trigger_delete<R: Runtime>(
    app: AppHandle<R>,
    ...
    delete_trigger_config(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    emit_config_update_and_snapshot(&app, &core, &state).await
}
...
#[tauri::command]
pub async fn desktop_tools_delete<R: Runtime>(
    app: AppHandle<R>,
    ...
    delete_tools_config(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    emit_config_update_and_snapshot(&app, &core, &state).await
}
...
#[tauri::command]
pub async fn desktop_context_delete<R: Runtime>(
    app: AppHandle<R>,
    ...
    delete_context_config(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    emit_config_update_and_snapshot(&app, &core, &state).await
}
...
#[tauri::command]
pub async fn desktop_config_components_apply<R: Runtime>(
    app: AppHandle<R>,
    request: crate::types::ConfigComponentsApplyRequest,
    state: State<'_, DesktopAppState>,
) -> Result<DesktopClientSnapshot, BridgeError> {
    let Some(core) = current_core(&state) else {
        return Err(BridgeError::untyped("desktop client is not running"));
    };
    crate::commands::apply_config_components(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    emit_config_update_and_snapshot(&app, &core, &state).await
}
...
#[tauri::command]
pub async fn desktop_config_components_patch<R: Runtime>(
    app: AppHandle<R>,
    request: crate::types::ConfigComponentsPatchRequest,
    state: State<'_, DesktopAppState>,
) -> Result<DesktopClientSnapshot, BridgeError> {
    let Some(core) = current_core(&state) else {
        return Err(BridgeError::untyped("desktop client is not running"));
    };
    crate::commands::patch_config_components(core.as_ref(), request)
        .await
        .map_err(|error| BridgeError::untyped(error.to_string()))?;
    emit_config_update_and_snapshot(&app, &core, &state).await
}
