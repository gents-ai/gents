...
pub fn desktop_workspace_list(
    ...
    let target = requested.canonicalize().map_err(|error| {
        BridgeError::untyped(format!("cannot open {}: {error}", requested.display()))
    })?;
    if !target.starts_with(&root) {
        return Err(BridgeError::untyped("path escapes the workspace root"));
    }

    let mut entries = Vec::new();
    let mut truncated = false;
    let read_dir = std::fs::read_dir(&target).map_err(|error| {
        BridgeError::untyped(format!("cannot list {}: {error}", target.display()))
    })?;
...
fn workspace_root(state: &State<'_, DesktopAppState>) -> Result<PathBuf, BridgeError> {
    ...
    let root = std::fs::read(agent_home.join("init.json"))
        ...
        .ok_or_else(|| {
            BridgeError::untyped("this agent has no tool root configured (meta-only ceiling)")
        })?;
    Path::new(&root).canonicalize().map_err(|error| {
        BridgeError::untyped(format!("tool root {root} is not accessible: {error}"))
    })
}
