...
fn session_snapshot_is_agent_scoped_when_session_ids_match() {
    let store = ClientStore::from_rows(ClientStoreRows {
        sessions: vec![
            ...
            AgentSession {
                session_id: "shared-session".to_string(),
                agent_did: "did:test:mini-2".to_string(),
                requester_did: None,
                behavior_id: "default".to_string(),
                created_at: "2026-04-21T12:01:00Z".to_string(),
                closed_at: None,
                title: Some(SessionTitle {
                    text: "mini-2 run".to_string(),
                    source: SessionTitleSource::User,
                }),
                tags: Vec::new(),
                provenance: None,
...
fn session_snapshot_exposes_pending_turn_when_latest_request_is_not_materialized() {
    let store = ClientStore::from_rows(ClientStoreRows {
        sessions: vec![AgentSession {session_id: "session-1".to_string(), agent_did: "did:test:amy".to_string(), requester_did: None, behavior_id: "amy-default".to_string(), created_at: "2026-04-21T12:00:00Z".to_string(), closed_at: None, title: Some(SessionTitle {text: "architecture-review".to_string(), source: SessionTitleSource::Generated}), tags: Vec::new(), provenance: None, observation: Some(SessionObservation {last_activity_at: "2026-04-21T12:02:00Z".to_string(), preview: Some("follow up question".to_string()), latest_request: Some(SessionRequestObservation {request_doc_id: "req-2".to_string(), request_id: "req-2".to_string(), lifecycle_state: RequestLifecycleState::Processing})})}],
        requests: vec![
            AgentRequestRow {
...
fn unbound_replica_forks_do_not_swallow_a_later_pending_turn() {
    let store = ClientStore::from_rows(ClientStoreRows {
        sessions: vec![AgentSession {
            ...
            observation: Some(SessionObservation {
                last_activity_at: "2026-04-21T12:02:00Z".to_string(),
                preview: Some("same prompt".to_string()),
                latest_request: Some(SessionRequestObservation {
                    request_doc_id: "req-2".to_string(),
                    request_id: "req-2".to_string(),
                    lifecycle_state: RequestLifecycleState::Processing,
