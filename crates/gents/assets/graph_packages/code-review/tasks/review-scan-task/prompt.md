...
Set each `finding_id` to `{{ doc.area_id }}:<finding-slug>`. Never retry a successful write. Then call `write_scan_result` exactly once as the final evidence write. Do not supply runtime-filled `run_id`, `area_id`, or `expected_total`. After that terminal scan result succeeds, call `update_goal` with `status="complete"`. Never complete the goal while an evidence read, required candidate write, or the terminal scan result remains unfinished; leave it active for continuation instead.
