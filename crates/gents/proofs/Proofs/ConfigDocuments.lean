...
def documentSpec : Collection → DocumentSpec
  ...
  | .agentContext => ⟨"AgentContext", "context_id", "behavior", ["context_id", "agent_did", "scope_behavior_id", "display_name", "description", "system_prompt", "tools_id", "compaction_id", "skill_ids", "tags"]⟩
  | .compaction => ⟨"CompactionConfig", "compaction_id", "profile", ["compaction_id", "agent_did", "scope_behavior_id", "display_name", "strategy", "threshold", "keep_recent_tokens", "tool_result_max_chars", "summary_max_output_tokens", "summary_file_list_max", "inference_profile_id", "tags"]⟩
  ...
  | .tools => ⟨"Tools", "tools_id", "tools", ["tools_id", "agent_did", "scope_behavior_id", "display_name", "host", "remote", "subagents", "built_ins", "datastore", "integrations", "self_config", "tags"]⟩
  ...
  | .inferenceProfile => ⟨"InferenceProfile", "profile_id", "profile", ["agent_did", "profile_id", "scope_behavior_id", "display_name", "description", "backend_id", "model_name", "reasoning_effort", "context_window", "max_output_tokens", "sampling_id", "execution_id", "tags"]⟩
  | .inferenceSampling => ⟨"InferenceSampling", "sampling_id", "profile", ["agent_did", "sampling_id", "scope_behavior_id", "display_name", "temperature", "top_p", "top_k", "seed", "min_p", "frequency_penalty", "presence_penalty", "repetition_penalty", "tags"]⟩
  | .inferenceExecution => ⟨"InferenceExecution", "execution_id", "profile", ["agent_did", "execution_id", "scope_behavior_id", "display_name", "max_turns", "max_total_tokens", "stream_batch_ms", "stream_liveness_timeout_secs", "deadline_duration_secs", "retry_policy_id", "tags"]⟩
  | .inferenceRetryPolicy => ⟨"InferenceRetryPolicy", "retry_policy_id", "profile", ["agent_did", "retry_policy_id", "scope_behavior_id", "display_name", "max_transport_retries", "backoff_ms", "max_resample_retries", "allow_repair", "interactive_max_retries", "tags"]⟩
