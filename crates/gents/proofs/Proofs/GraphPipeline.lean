  ...
  | .succeedRun resultContractSatisfied activeWorkTerminal =>
      ...
      | some run =>
          if run.status = .running ∧ resultContractSatisfied = true ∧
            ...
            none
      | none => none
  | .failRun failureProven activeWorkTerminal =>
      if failureProven = true ∧ activeWorkTerminal = true then
...
theorem failure_requires_durable_evidence
    {pre post : State} {failureProven activeWorkTerminal : Bool}
    (h : step? pre (.failRun failureProven activeWorkTerminal) = some post) :
    failureProven = true ∧ activeWorkTerminal = true := by
  simp only [step?] at h
  split at h <;> simp_all
...
theorem terminal_run_rejects_further_terminal_writes
    ...
    step? state (.succeedRun true true) = none ∧
      step? state (.failRun true true) = none ∧
