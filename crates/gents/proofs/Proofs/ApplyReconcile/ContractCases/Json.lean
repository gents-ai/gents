...
def applyReconcileCaseJson (witness : ApplyReconcileCase) : String :=
  "{"
    ...
    ++ "\"prune_mode\":" ++ boolString witness.pruneMode ++ ","
    ...
    ++ "\"expected_delete\":"
      ++ jsonArray (witness.expectedDelete.map docRefJson) ++ ","
    ++ "\"expected_unchanged\":"
    ...
    ++ "\"expected_prune_order\":"
      ++ jsonArray (witness.expectedPruneOrder.map collectionWriteJson) ++ ","
    ...
    ++ "\"expected_selected_delete_docs\":"
      ++ jsonArray (witness.expectedSelectedDeleteDocs.map selectedDocJson) ++ ","
    ...
    ++ "\"prune_order_referrers_before_dependencies\":"
      ++ boolString witness.pruneOrderReferrersBeforeDependencies ++ ","
    ...
    ++ "\"delete_safety_holds\":"
      ++ boolString witness.deleteSafetyHolds
    ++ "}"
