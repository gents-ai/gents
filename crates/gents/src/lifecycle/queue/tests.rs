...
#[tokio::test]
async fn background_notification_is_atomically_bound_to_coalesced_wake() {
    let db = test_db("atomic-background-notification").await;
    let mut parent = parent_request("atomic-background-session");
    ...
    let queue_hints = QueueHints {
        ...
        key: Some("background_completion:atomic-background-session".to_string()),
