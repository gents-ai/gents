...
use crate::graph_pipeline::{
    ...
    PackageArtifactKind, PackageBindingValue, PackagePlan, PackageRoleBinding,
    PlannedPackageArtifact, RequiredSchemaDigest, StageCapability,
};
...
pub struct GraphPackageInstallBindings {
    ...
    #[serde(default)]
    pub variables: BTreeMap<String, PackageBindingValue>,
}
...
pub async fn prepare_bundled_graph_package_install(
    ...
    for template in &package.capabilities {
        ...
        ceilings.insert(template.capability_id.clone(), template.authority.clone());
    }
    ...
    let mut package_plan = PackagePlan {
        ...
        catalog_digest: package.catalog_digest.clone(),
        ...
        bindings: bindings.variables.clone(),
        roles: bindings.roles.clone(),
        effective_authority_ceiling: ceilings,
        predecessor_revision_digest: None,
        artifacts,
