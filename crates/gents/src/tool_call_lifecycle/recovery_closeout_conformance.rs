...
async fn queued_subagents_of_terminal_local_and_foreign_parents_stay_pending() {
    ...
    let response = admission.node.execute(&format!(r#"mutation {{ create_AgentRequest(input: {{ request_id: "{bystander}", purpose: "normal", agent_did: "{local_did}", behavior_id: "general", session_id: "{local_session}", retry_parent_request: "", retry_root_request: "{bystander}", superseded_by_request: "", content: "wake", lifecycle_state: "pending", backend_id: "", execution_origin: "scheduled", created_at: "2026-03-23T00:00:00Z", retry_count: 0, max_retries: 3, subagent_depth: 1, caused_by_parent_request_id: "{local_parent}", caused_by_parent_tool_call_id: "{local_tool}" }}) {{ _docID }} }}"#)).await;
    assert!(!response.has_errors(), "{:?}", response.errors);
