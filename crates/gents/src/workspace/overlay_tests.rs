...
async fn generated_artifact_admission_cases_drive_live_binding_and_launch_policy() {
    ...
    for case in cases {
        ...
        if name == "integrate_not_artifact" {
            // Integrate is an executor authority, not an admissible request
            // binding. Fence the real signing boundary rather than forge one.
            assert_eq!(case["expected_admitted"], false);
            assert!(case["expected_bound_mode"].is_null());
            let error = alternate.err().expect("Integrate request must be rejected");
            assert!(error
                .to_string()
                .contains("unsupported workspace_authority"));
            continue;
        }
