...
use super::types::{
    ...
    PortCardinality, PortRef, PortSpec, ResultCardinality, ResultPredicate, StageCapability,
    COMPILER_VERSION,
};
use crate::graphql::{
...
pub fn compile_graph(
    ...
    let declared_result_names = intent
        .results
        .iter()
    ...
    for (index, result) in intent.results.iter().enumerate() {
        ...
        for (predicate_index, predicate) in result.predicates.iter().enumerate() {
            let mut fields = Vec::new();
            let referenced_result = match predicate {
                ...
                | ResultPredicate::FieldEqualsField {
                    ...
                    fields.extend([field, result_field]);
                    Some(result)
                }
                ...
                ResultPredicate::FieldEqualsSum { field, terms } => {
                    ...
                    if terms.len() < 2 {
                        diagnostic(
                            &mut diagnostics,
                            DiagnosticCode::InvalidResultPredicate,
                            format!("{path}/predicates/{predicate_index}/terms"),
                            "field_equals_sum requires at least two term fields",
                        );
                    }
                    None
                }
            ...
            for field in fields {
                if validate_graphql_name(field).is_err() {
                    diagnostic(
                        &mut diagnostics,
                        DiagnosticCode::InvalidResultPredicate,
                        format!("{path}/predicates/{predicate_index}/field"),
                        "result predicate fields must be GraphQL field identifiers",
                    );
                }
            }
            if referenced_result.is_some_and(|name| !declared_result_names.contains(name.as_str()))
            {
                diagnostic(
                    &mut diagnostics,
                    DiagnosticCode::InvalidResultPredicate,
                    format!("{path}/predicates/{predicate_index}/result"),
                    "result predicate references an unknown named result",
                );
            }
        }
    }
    ...
    let mut results: Vec<PlannedResult> = intent
        ...
        .map(|result| {
            ...
            PlannedResult {
                ...
                predicates: result.predicates.clone(),
            }
        })
        .collect();
