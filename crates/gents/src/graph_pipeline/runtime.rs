...
async fn activate_in_txn(
    ...
    let plan_json = revision
        .get("plan_json")
        .and_then(Value::as_str)
        .context("candidate revision is missing plan_json")?;
    let plan: GraphPlan = serde_json::from_str(plan_json)?;
...
async fn start_run_in_txn(
    ...
    let plan_json = revision
        .get("plan_json")
        .and_then(Value::as_str)
        .context("active graph revision is missing plan_json")?;
    let plan: GraphPlan = serde_json::from_str(plan_json)?;
    if plan.digest != digest || plan.graph_id != graph_id || !verify_graph_plan_digest(&plan) {
