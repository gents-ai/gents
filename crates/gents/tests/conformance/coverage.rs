...
use crate::lean_vocab_test::{
    lean_canonical_dispatch_observation_cases, lean_canonical_execution_gate_cases,
...
fn lean_contract_coverage_ledger_accounts_for_every_emitted_domain() {
    ...
    if !lean_canonical_dispatch_observation_cases().is_empty() {
        emitted.insert((
            "canonical_dispatch_observation_cases".to_string(),
            "CanonicalDispatchObservationCases".to_string(),
        ));
    }
    if !lean_canonical_spawned_target_rejection_cases().is_empty() {
        emitted.insert((
            "canonical_spawned_target_rejection_cases".to_string(),
            "CanonicalSpawnedTargetRejectionCases".to_string(),
        ));
    }
