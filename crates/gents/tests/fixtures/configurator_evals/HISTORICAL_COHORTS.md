# Historical progressive configurator cohorts

This is the retained record of the earlier Pagoda-based workflow, not the current
runner contract. Its artwork/review/improvement stages, fixtures and browser
grader have been retired. Old commands and stage counts below describe those
historical cohorts; use README.md for current commands and coverage. Private raw
cohort evidence has not been changed or deleted.
...
Builder readiness checks the command runtime's recorded argv, cwd and exit status,
alongside the exact script and resulting artifact. The fixture requests a separate
direct script invocation; the grader never interprets shell programs or model
tool arguments. Historical shell-specific reassessment code has been retired;
old receipts remain unchanged. Both suites check preview configuration/schema
snapshots and runtime config execution receipts, not command spellings.

The runner writes runtime output directly to its log. A stale launcher heartbeat
is reported as stalled, not active work or a model failure. Failed tool and provider
observations together remain unknown unless the causal boundary can be established.
