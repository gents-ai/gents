...
/* the two subscription kinds, and the provider name their account carries */
const SUBSCRIPTION: Record<
  string,
  { provider: string; title: string; note: string; login: 'codex' | 'grok' }
> = {
  ChatGptCodex: {
    provider: 'chatgpt',
    title: 'ChatGPT / Codex',
...
function useAccounts(shell: Shell, agentDid: string) {
  ...
  const api = shell.api
  const load = () =>
    (api.listProviderAccounts?.(agentDid) ?? Promise.resolve([])).then(setAccounts, () =>
      setAccounts([]),
    )
  useEffect(() => {
    void load()
...
function Editor({
  ...
  const base = { name: 'agent' as const, agentDid: deployment.agentDid, section: 'inference' }
  const saved = {
    name: backend.name ?? '',
    ...
    <>
        ...
        action={
          <span className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                setProbe('probing…')
                const r = await shell.api.probeInferenceEndpoint(d.draft.endpoint).catch(() => null)
                setProbe(
                  r
                    ? r.reachable
      ...
      >
        ...
        <ChoiceRow
          ...
          items={KINDS}
        />
        <FactRow label="Used by" description="Delete is blocked while a behaviour points here.">
