...
function SkillEditor({
  ...
  const base = { name: 'agent' as const, agentDid: deployment.agentDid, section: 'skills' }
  const saved = {
    name: skill.name ?? '',
  ...
  return (
    <>
      ...
      <DeleteButton
        ...
        onDelete={() =>
          shell.applyConfig((api) =>
            api.deleteSkillConfig({ skillId: skill.skillId, agentDid: deployment.agentDid }),
          )
        }
      />
    </>
  ...
  return (
    <ListDetail
      ...
      onCreate={async () => {
        ...
        navigate({ name: 'agent', agentDid: deployment.agentDid, section: 'skills', item: skillId })
      }}
      detail={(id) => {
