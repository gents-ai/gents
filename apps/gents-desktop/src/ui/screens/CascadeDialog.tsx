...
export function CascadeDialog({
  ...
  const count = (preview?.willInterrupt.length ?? 0) + (preview?.willDetach.length ?? 0)
  return (
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {!preview ? (
              ...
              <>
                {changed && (
                  <span className="block text-foreground">The tree changed; check again.</span>
                )}
                {count === 0
          ...
          <ul className="grid gap-1 text-sm">
            {preview.willInterrupt.map((c) => (
              <li key={c.requestId} className="flex items-center justify-between gap-3">
                ...
                <span className="font-mono text-[11px] text-muted-foreground">{c.requestId}</span>
              </li>
            ))}
          </ul>
        )}
