            ...
            <ul className="px-3">
              {recent.map((c) => (
                <li key={c.sessionId}>
                  ...
                  >
                    <SessionStatus turnState={c.turnState} held={holds.has(c.sessionId)} />
                    <span className="min-w-0 flex-1 truncate">{c.title ?? 'Untitled session'}</span>
                  </a>
                </li>
              ))}
            </ul>
...
export function RailFlyout({
  ...
  return (
    <div
      ...
      onPointerEnter={(e) => hover && e.pointerType === 'mouse' && later(true, OPEN_AFTER)}
      onPointerLeave={() => hover && later(false, CLOSE_AFTER)}
      onFocus={() => hover && later(true, 0)}
      onBlur={(e) => {
        if (hover && !e.currentTarget.contains(e.relatedTarget as Node | null)) later(false, 0)
      }}
