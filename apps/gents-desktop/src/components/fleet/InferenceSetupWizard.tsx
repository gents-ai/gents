...
const LOCAL_PROBE_URLS = ["http://127.0.0.1:8080/v1", "http://127.0.0.1:11434/v1"];
...
export function InferenceSetupWizard({
  ...
  async function persistBackend(opts: {
    ...
    if (targets.behavior) {
      await onSaveBehaviorConfig(
        behaviorSaveFrom(targets.behavior, deployment.agentDid, targets.backendId),
      );
    }
  }
  ...
  async function signInWithChatGpt() {
    ...
    try {
      ...
      unlisten = await listen<{ url: string }>("desktop://codex-login-url", (event) =>
        setCodexAuthUrl(event.payload?.url ?? null),
      );
  ...
  return (
    <div className="dialog-backdrop open" role="presentation" onClick={cancelAndClose}>
      <div
      ...
      >
        ...
        <div className="inference-wizard-body">
          ...
          {error ? (
            <p className="inference-wizard-error" data-testid="inference-wizard-error">
              {error}
            </p>
          ) : null}
            ...
            <div className="inference-wizard-form">
              ...
              <div className="inference-wizard-actions">
                <button
                  ...
                  type="button"
                  disabled={submitting || !openaiKey.trim() || !openaiModel.trim()}
            ...
            <div className="inference-wizard-form">
              ...
              <div className="inference-wizard-actions">
                <button
                  ...
                  type="button"
                  disabled={submitting || !localUrl.trim() || !localModel.trim()}
            ...
            <div className="inference-wizard-form">
              ...
              <div className="inference-wizard-actions">
                <button
                  ...
                  type="button"
                  disabled={submitting || !customUrl.trim() || !customModel.trim()}
            ...
            <div className="inference-wizard-form">
              ...
              {submitting && codexAuthUrl ? (
                <p className="muted small">
                  ...
                  <span className="mono inference-wizard-authurl">{codexAuthUrl}</span>
                </p>
              ) : null}
              <div className="inference-wizard-actions">
                {signingIn ? (
...
function ChooseStep({
  ...
  return (
    <ul className="inference-wizard-options" data-testid="inference-wizard-options">
      <OptionCard
...
function LocalDetectionHint({ detection }: { detection: Detection }) {
  if (detection.status === "found") {
    return <p className="muted small">Detected a running server at {detection.url}.</p>;
  }
  if (detection.status === "none") {
    return (
      <p className="muted small">
        No local server detected. Start one (Ollama or llama-server), then Detect.
      </p>
    );
  }
  return null;
