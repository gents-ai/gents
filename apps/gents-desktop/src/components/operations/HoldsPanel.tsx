...
import { resolveToolCallHold } from "../../lib/desktop-api";
...
export function HoldsPanel({ agentDid }: { agentDid: string | null }) {
  ...
  const resolve = async (hold: HeldToolCallView, approve: boolean, reason?: string) => {
    if (!agentDid) {
      return;
    }
  ...
  return (
    <section className="holds-panel" data-testid="holds-panel">
      ...
      {loading && holds == null ? <p className="holds-panel-empty">Loading…</p> : null}
