...
use super::super::types::ClientUpdateEvent;
...
) -> Result<InferenceProbeResult, String> {
...
#[tauri::command]
pub(crate) async fn desktop_codex_login(
    app: AppHandle,
    ...
    let server = run_login_server(server_opts)
        .map_err(|error| format!("starting ChatGPT login server: {error}"))?;
    let _ = app.emit(
    ...
    match wait {
        Ok(result) => {
            result.map_err(|error| format!("ChatGPT browser login failed: {error}"))?;
        }
        Err(_elapsed) => {
            // Stop the still-listening server so its port is released, then bail.
            cancel.shutdown();
            return Err("ChatGPT sign-in timed out waiting for the browser".to_string());
        }
    }
    ...
    let token_data = auth
        .get_token_data()
        .map_err(|error| format!("ChatGPT login did not expose token data: {error}"))?;
    ...
    let doc_id = upsert_oauth_credential(&node, &credential)
        .await
        .map_err(|error| format!("storing ChatGPT credential: {error}"))?;
...
#[tauri::command]
pub(crate) fn desktop_codex_login_cancel(state: State<'_, DesktopAppState>) -> Result<(), String> {
    let handle = {
        let mut bridge = state
            .bridge
            .lock()
            .map_err(|_| "desktop bridge lock poisoned".to_string())?;
