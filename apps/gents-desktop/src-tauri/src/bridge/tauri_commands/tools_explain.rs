...
/// desktop cannot read.
#[tauri::command]
pub(crate) fn desktop_tool_surface_explain(
    agent_did: String,
    behavior_id: String,
    ...
    tauri::async_runtime::block_on(async move {
        if core.graphql_for_agent(&agent_did).await.is_some() {
            return Err(
                "tool-surface explanation for remote agents is not yet supported".to_string(),
            );
        }
        ...
        let behavior = snapshot
            ...
            .iter()
            .find(|row| {
                row.behavior_id == behavior_id && row.agent_did.as_deref() == Some(&agent_did)
            })
            .ok_or_else(|| format!("behavior {behavior_id} not found for {agent_did}"))?;

        let (ceiling, ceiling_source) = resolve_desktop_tool_ceiling()?;
        ...
        let active_behavior_ids = snapshot
            ...
            .iter()
            .filter(|row| {
                row.agent_did.as_deref() == Some(&agent_did) && row.enabled.unwrap_or(true)
            })
        ...
        let (tool_selection_source, config, tool_policy_version) = match tool_selection_id
            .as_deref()
        {
            Some(selection_id) => {
                let row = snapshot
...
fn resolve_desktop_tool_ceiling() -> Result<(ToolCeiling, &'static str), String> {
