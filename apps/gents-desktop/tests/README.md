...
- `npm run perf:mobile -- --runs=5` measures deterministic mobile interaction
  ...
  `test-results/mobile-performance/`; only deterministic structural assertions
  fail the command by default. Add `--enforce-responsive-budgets` on a controlled
  host to also gate the retained long-transcript typing budgets. Elapsed time,
  React work, JavaScript heap, and CPU-task proxies remain recorded as evidence.
  See [CHAT_RESPONSIVENESS.md](./CHAT_RESPONSIVENESS.md)
