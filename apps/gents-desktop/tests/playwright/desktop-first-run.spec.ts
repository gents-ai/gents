...
test.describe("first-run install", () => {
  test("creates a local agent, adds inference, and starts a conversation", async ({
    ...
    await expect(page.getByRole("heading", { name: "Configure your agent" })).toBeVisible();
    await expect(page.getByRole("textbox", { name: "Agent name" })).toHaveValue("Forge");

    await page.getByTestId("setup-next").click();
    await expect(page.getByRole("heading", { name: "Startup" })).toBeVisible();

    await expect(page.getByRole("radiogroup", { name: "Inference provider" })).toBeVisible({
      timeout: 15_000,
    });
