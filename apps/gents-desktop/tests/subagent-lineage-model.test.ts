...
import {
  ...
  splitNodeId,
  subtreeHasSurvivor,
} from "../src/components/subagentLineage/lineageModel";
import type { SubagentTreeView } from "../src/lib/types";
...
describe("lineageModel", () => {
  ...
  it("flattens only expanded branches", () => {
    ...
    expect(flattenTreeOrder(root, new Set()).map(nodeId)).toEqual(["req:req-root"]);
    expect(flattenTreeOrder(root, new Set(["req:req-root"])).map(nodeId)).toEqual([
      "req:req-root",
      "tool:tool-1",
    ]);
  });
