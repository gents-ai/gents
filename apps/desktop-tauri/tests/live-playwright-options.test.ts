...
describe("live Playwright option resolution", () => {
  it("uses mock inference by default when no live backend is configured", () => {
    ...
    expect(options.env.DEFRA_AGENT_TAURI_LIVE).toBe("1");
    ...
    expect(env.DEFRA_AGENT_TAURI_LIVE_INFERENCE_URL).toBe("http://127.0.0.1:1234/v1");
  ...
  it("fails loudly in real-provider mode when no backend is configured", () => {
    ...
    expect(() =>
      resolveLivePlaywrightOptions(["--list"], {
        DEFRA_AGENT_TAURI_LIVE_REQUIRE_REAL_INFERENCE: "1",
      }),
    ).toThrow(/real-inference mode requires a backend/);
  });
  ...
  it("accepts explicit real-provider flags and forwards remaining Playwright args", () => {
    ...
    expect(options.env.DEFRA_AGENT_TAURI_LIVE_INFERENCE_URL).toBe(
      "https://api.example.test/v1",
    );
    expect(options.env.DEFRA_AGENT_DESKTOP_LIVE_BACKEND_ENDPOINT).toBe(
      "https://api.example.test/v1",
    );
    expect(options.env.DEFRA_AGENT_TAURI_LIVE_MODEL_NAME).toBe("provider-model");
    expect(options.env.DEFRA_AGENT_TAURI_LIVE_PROVIDER).toBe("openai-compatible");
    expect(options.env.DEFRA_AGENT_TAURI_LIVE_API_KEY_ENV_VAR).toBe("OPENAI_API_KEY");
  });
  ...
  it("uses existing live backend env vars without enabling mock inference", () => {
    ...
    expect(options.env.DEFRA_AGENT_TAURI_LIVE_MODEL_NAME).toBe("MiniMax-M2.7-NVFP4");
  });
