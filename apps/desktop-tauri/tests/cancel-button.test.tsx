...
describe("CancelButton", () => {
  it("renders nothing when no turn is in flight", () => {
    const { container } = render(
      <CancelButton activeRequestId="req_a17" turnState="idle" onInterruptClick={() => {}} />,
    );
  ...
  it("renders enabled when a turn is streaming and activeRequestId is known", () => {
    render(
      <CancelButton activeRequestId="req_a17" turnState="streaming" onInterruptClick={() => {}} />,
    );
  ...
  it("renders disabled with waiting-for-turn copy when turn active but activeRequestId is null", () => {
    render(
      <CancelButton activeRequestId={null} turnState="streaming" onInterruptClick={() => {}} />,
    );
    const btn = screen.getByRole("button", { name: /interrupt/i });
    expect(btn).toBeDisabled();
    expect(btn).toHaveAttribute("title", expect.stringMatching(/waiting for turn to register/i));
  });

  it("calls onInterruptClick when clicked", () => {
    const handler = vi.fn();
    render(
      <CancelButton activeRequestId="req_a17" turnState="streaming" onInterruptClick={handler} />,
    );
  ...
  it("does not call onInterruptClick when disabled", () => {
    const handler = vi.fn();
    render(
      <CancelButton activeRequestId={null} turnState="streaming" onInterruptClick={handler} />,
    );
    fireEvent.click(screen.getByRole("button", { name: /interrupt/i }));
