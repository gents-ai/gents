  ...
  it("configures backend profile tools behavior task and runs it", async () => {
    ...
    try {
      ...
      await waitForDeploymentDocument(runner, (current) => {
        expect(
          current.inferenceBackends.some((backend) => backend.backendId === backendId),
        ).toBe(true);
      });
      ...
      await waitForDeploymentDocument(runner, (current) => {
        expect(
          current.inferenceProfiles.some((profile) => profile.profileId === profileId),
        ).toBe(true);
      });
      ...
      logTurn(`task run submitted taskId=${taskId} requestId=${taskRun.requestId}`);
