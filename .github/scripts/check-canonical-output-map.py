...
MAPS = [f"contracts/canonical-output-map-{area}.json"
        for area in ("execution", "projection", "native")]
...
def constructors(source, declaration):
    """Read the one-constructor-per-line style of the three closed vocabularies.
...
def main():
    ...
    try:
        ...
        if args.check_export:
            ...
            subprocess.run(["lake", "env", "lean", "--stdin"], input=checks,
    ...
    if errors:
        ...
        return 1
    print(f"Checked {len(entries)} handoff entries and all three constructor inventories; "
