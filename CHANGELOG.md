...
- A durable Goal no longer spends its infrastructure retries while its behavior
  is unavailable. Continuation waits for the runtime's behavior readiness; a
  continuation rejected before execution because the behavior was unavailable
  is re-issued without a charge; and a behavior that stays unavailable after
  reconciliation settles pauses the Goal with that reason instead of waiting
  forever (#1345).
