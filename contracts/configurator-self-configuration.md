...
The protected Configurator (`gents:base:configurator`) and cross-scoped
Context/Tools references are rejected rather than
...
Configurator and generated working-role instructions prohibit searching/adopting another
...
Configurator, CLI, desktop, and mobile therefore continue through the same admission
...
| `behavior_id` | Exact same-principal `AgentBehavior` target. New personal behaviors allocate `local:{slug}` and deterministic component IDs; callers reuse the returned ID exactly. | Required for inspect/edit/disable; never inferred from display name. |
| `clone_from` | Exact enabled source behavior. Its complete mutable context and inference closure is copied into independently scoped documents; reusable backends/resources remain references. | Required for clone; incompatible with a preset in the signed create intent. |
| `display_name` | Writes the independent `AgentBehavior.display_name`; changing it never changes the logical or component IDs. | Required and 1–64 characters for create; optional sparse update for edit. Duplicate display names are allowed. |
...
| `profile_id` | Selects an exact same-principal profile as the source for an independently scoped inference copy; the backend connection remains shared. | Required for create/clone; optional sparse replacement for edit and never clearable. |
...
| --- | --- |
| Coding behavior | Preview admits a complete prompt; create returns the allocated `local:{slug}` behavior ID and its materialized component IDs; display name, descriptions, and literal prompt match; effective root is inside the process ceiling; principal default equals the returned ID; a new session is explicitly bound to it. |
...
| Rejected authority/root | A path outside the process ceiling or published narrowing roots produces a rejected/no-write outcome naming valid choices; Configurator and the target documents are byte-equivalent before/after. |
