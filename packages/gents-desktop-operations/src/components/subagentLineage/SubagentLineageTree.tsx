...
import type { AnyNode, FilterState } from "./lineageModel.js";
import { nodeId, shortenDid, subtreeHasSurvivor } from "./lineageModel.js";
...
function nodeSecondary(node: AnyNode): string {
  ...
  const bits = [
    ...
    [node.edge.awaitMode, node.edge.cancelPolicy].filter(Boolean).join("/") ||
      null,
  ].filter(Boolean);
  return bits.join("  ·  ");
}
