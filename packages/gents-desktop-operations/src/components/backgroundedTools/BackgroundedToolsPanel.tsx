...
export type BackgroundedToolsPanelProps = {
  ...
  runtime?: import("@source-inc/gents-desktop-client").RuntimeView | null;
...
export function BackgroundedToolsPanel({
  ...
  const filtered = useMemo(() => {
    const rows = projected.filter((r) => {
      ...
      if (stateFilters.size > 0 && !stateFilters.has(r.derivedState))
        return false;
      if (
        awaitFilters.size > 0 &&
            ...
            {filtered.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td>
                    <span
                      className="pill pill-await"
                      data-mode={row.awaitMode ?? ""}
                    >
                      {row.awaitMode ?? "—"}
                    </span>
                  </td>
