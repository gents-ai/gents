...
4. Run the bundled `code_review` graph against the open PR through `gents graph run code_review`, using the maintenance worktree as `--repo` and the declared base/head. Require multiple concern lenses and inspect its durable graph result rather than relying on process exit alone.
