...
Include `owned_files` on the report as a JSON-array string of exact repository-relative file paths for the entire planned commit series, including intended new files and deletions. Resolve concrete files from the verified findings before publishing; directory names, globs, absolute paths, and traversal are invalid. This immutable manifest grants the shared workspace its file-write scope and cannot be expanded during execution. If the required paths cannot be determined, use the skipped sentinel instead of guessing or granting the whole repository. A skipped report uses the explicit string `[]`.
