...
Every `gents provision` or `gents config apply` invocation used to build
...
First run `python3 packs/grok_tui_port/scripts/grok_edge_probe.py --edge offline` and
`python3 packs/grok_tui_port/scripts/grok_stock_pty_probe.py self-test`; require both
embedded validator matrices to pass without a socket. Before starting the server, run
`python3 packs/grok_tui_port/scripts/grok_stock_pty_probe.py preflight` for `live_graphql` and `live_socket` and
...
`python3 packs/grok_tui_port/scripts/grok_stock_pty_probe.py run` against the stock
...
`python3 packs/grok_tui_port/scripts/grok_stock_pty_probe.py cleanup` against the saved proof and require both the
