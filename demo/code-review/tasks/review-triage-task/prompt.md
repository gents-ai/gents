...
Call `defra_query` for `FindingVerdict` with `run_id == "{{ event.correlation }}"`. Check that its counts agree with the source summary. For every row whose verdict is exactly `confirmed`, call `write_finding` preserving all content, evidence, and verification fields. Do not call `write_finding` for `refuted` rows.
