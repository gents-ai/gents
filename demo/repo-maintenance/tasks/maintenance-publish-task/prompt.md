...
4. Run the bundled `code-review` graph against the open PR through `gents graph run code-review`, using the maintenance worktree as `--repo` and the declared base/head. Require multiple concern lenses and inspect its durable graph result rather than relying on process exit alone.
