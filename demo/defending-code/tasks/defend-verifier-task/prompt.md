...
Otherwise call `read_defense_threat_model` and `read_defense_candidate` once
each. The candidate read is restricted to this run and exact finding.
...
`source_revision` and `source_tree_state`. If HEAD changed after bootstrap,
adjudicate the clean recorded revision with Git object reads or a disposable
exact-base clone and call the mismatch out in `verification`. Set
`duplicate_of=none`; clustering owns consequence collapse.
...
guards. A refuted claim uses `severity=NONE`; a confirmed claim uses exactly
`HIGH`, `MEDIUM`, or `LOW`.
