...
1. About five files per batch where the batch cap allows; when the
   candidate count forces larger batches, balance file counts evenly.
   Group related files: same slug family, same module or subsystem, or
   the same vulnerability class. Precise-tier candidates lead: they get
   the earliest batch ids and the tightest grouping. Never mix a
   precise-tier file into a batch of purely noisy candidates when a
   tighter grouping exists.
...
3. If there are fewer candidate files than the minimum batch count, pad
   the batch list to the minimum by assigning the highest-risk unflagged
   source directories (authorization, persistence, and provider-boundary
   code first), and say in each padded batch's instructions that its
   files carry no pre-scan hits.
4. You may use read-only file tools to check a path exists or gauge a
   file's size when deciding a grouping; do not read files to pre-judge
   findings, and never use a shell.
5. Decide the full batch list first, then write every batch. For each
...
6. Do not supply `run_id`; it is runtime-filled. Do not finish until every
